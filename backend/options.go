...
var optionCatalog = []Option{
	...
	{Key: "speculative_mode", Category: CategoryAdvanced, ValueKind: ValueChoice, ProfileField: "SpeculativeMode", Description: "Enable speculative decoding with native MTP capabilities or a separate registered draft model.", MemoryImpact: "Separate draft and assistant models consume additional accelerator or system memory."},
	{Key: "speculative_tokens", Category: CategoryAdvanced, ValueKind: ValueInteger, ProfileField: "SpeculativeTokens", Description: "Maximum number of speculative tokens proposed before target-model verification.", OmitWhenUnset: true, MemoryImpact: "Larger values increase verification work and may not improve performance when acceptance is low."},
	{Key: "draft_model_id", Category: CategoryAdvanced, ValueKind: ValueChoice, ProfileField: "DraftModelID", Description: "Registered model used as the speculative draft or MTP assistant.", MemoryImpact: "The selected model is loaded alongside the target model."},
